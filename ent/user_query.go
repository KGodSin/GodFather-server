...
func (uq *UserQuery) QueryApprovalInformations() *ApprovalInformationQuery {
	query := &ApprovalInformationQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withApprovalInformations: uq.withApprovalInformations.Clone(),
...
func (uq *UserQuery) WithApprovalInformations(opts ...func(*ApprovalInformationQuery)) *UserQuery {
	query := &ApprovalInformationQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withApprovalInformations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [1]bool{
			uq.withApprovalInformations != nil,
		}
	)
	...
	if query := uq.withApprovalInformations; query != nil {
		...
		query.Where(predicate.Approval_information(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ApprovalInformationsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_approval_informations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_approval_informations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_approval_informations" returned %v for node %v`, *fk, n.ID)
			}
