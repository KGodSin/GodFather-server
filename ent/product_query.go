...
type ProductQuery struct {
	...
	// eager-loading edges.
	withProductOrders        *ProductOrderQuery
	withApprovalInformations *ApprovalInformationQuery
...
// QueryProductOrders chains the current query on the "product_orders" edge.
func (pq *ProductQuery) QueryProductOrders() *ProductOrderQuery {
	query := &ProductOrderQuery{config: pq.config}
...
// QueryApprovalInformations chains the current query on the "approval_informations" edge.
func (pq *ProductQuery) QueryApprovalInformations() *ApprovalInformationQuery {
	query := &ApprovalInformationQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withProductOrders:        pq.withProductOrders.Clone(),
		withApprovalInformations: pq.withApprovalInformations.Clone(),
...
// WithProductOrders tells the query-builder to eager-load the nodes that are connected to
// the "product_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithProductOrders(opts ...func(*ProductOrderQuery)) *ProductQuery {
	query := &ProductOrderQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductOrders = query
	return pq
}

// WithApprovalInformations tells the query-builder to eager-load the nodes that are connected to
// the "approval_informations" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithApprovalInformations(opts ...func(*ApprovalInformationQuery)) *ProductQuery {
	query := &ApprovalInformationQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withApprovalInformations = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [2]bool{
			pq.withProductOrders != nil,
			pq.withApprovalInformations != nil,
		}
	)
	...
	if query := pq.withProductOrders; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Product)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ProductOrders = []*Product_order{}
		}
		query.withFKs = true
		query.Where(predicate.Product_order(func(s *sql.Selector) {
			s.Where(sql.InValues(product.ProductOrdersColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.product_product_orders
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "product_product_orders" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "product_product_orders" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ProductOrders = append(node.Edges.ProductOrders, n)
		}
	}

	if query := pq.withApprovalInformations; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Product)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ApprovalInformations = []*Approval_information{}
		}
		query.withFKs = true
		query.Where(predicate.Approval_information(func(s *sql.Selector) {
			s.Where(sql.InValues(product.ApprovalInformationsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.product_approval_informations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "product_approval_informations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "product_approval_informations" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ApprovalInformations = append(node.Edges.ApprovalInformations, n)
		}
	}
